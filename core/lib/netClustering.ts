...
(function() {
    ...
    netClustering.cluster = function (nodes, edges, clusterAttr, edgesCountAttr) {
        ...
        // dataObj.names = nodes;
        dataObj.distances = {};
        linksForClustering =  linksForClustering.filter(function (d) { return d.sourceId !== d.targetId; }); //avoid loops
        linksForClustering.forEach(function (d) {
            var hash = Math.min(d.sourceId, d.targetId) + "~" + Math.max(d.sourceId, d.targetId); // hash always has left key < right key})
